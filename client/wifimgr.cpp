...
#include <DNSServer.h>
#include <esp_wifi.h>
#include "led.h"   // PATCH: Add LED support
...
void clearOnNotFound() {
    // Re-register a minimal not found (404) handler.
...
void startPortal() {
    ...
    bool apok = WiFi.softAP("LoveByte Setup", NULL, 1, 0);
    ...
    Led::setMode(LedMode::BreathePink);   // PATCH: Pink breathe in portal mode
...
<body>
    <div class="container">
        <div style="width:100%;text-align:center;margin-bottom:1em">
            <span style="font-size:2em;font-weight:bold;">LoveByte Setup</span>
        </div>
        <form id="wifiForm">
    ...
    server.on("/connect", HTTP_GET, [](AsyncWebServerRequest *request){
        ...
        Led::setMode(LedMode::BootBlink);    // PATCH: Blink while connecting
    ...
    server.on("/forget", HTTP_GET, [](AsyncWebServerRequest *request){
        ...
        WiFi.disconnect();
        state = State::PORTAL;
        Led::setMode(LedMode::BreathePink); // PATCH: breathe pink after forget
    ...
    server.on("/debug/forget", HTTP_GET, [](AsyncWebServerRequest *request){
        ...
        WiFi.disconnect(true);
        state = State::PORTAL;
        Led::setMode(LedMode::BreathePink); // PATCH: breathe pink after debug/forget
    ...
    [](AsyncWebServerRequest *request, uint8_t* data, size_t len, size_t index, size_t total) {
        ...
        // Parse JSON manually (quick & dirty for /save endpoint)
    ...
    server.begin();
    state = State::PORTAL;
    Led::setMode(LedMode::BreathePink); // PATCH: ensure LED always correct at start of portal
}
...
void tryConnect() {
    if (ssid.length() > 0) {
        ...
        connectAttempts = 1;
        lastAttempt = millis();
        Led::setMode(LedMode::BootBlink); // PATCH: blink while trying to connect
    } else {
        startPortal();
    }
}
...
void loop() {
    ...
    if (state == State::CONNECTING) {
        if (WiFi.status() == WL_CONNECTED) {
            ...
            dnsServer.stop();
            WiFi.softAPdisconnect(true);
            Led::setMode(LedMode::RainbowFade);   // PATCH: rainbow when connected
            ...
            if (connectAttempts >= maxAttempts) {
                ...
                WiFi.begin(ssid.c_str(), password.c_str());
                lastAttempt = millis();
                Led::setMode(LedMode::BootBlink); // PATCH: blink while retrying
            }
        }
    }
}
